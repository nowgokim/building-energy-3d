...
| F1 | 밀집도·클러스터·소방서 | 2~3주 | 대기 | 클러스터 API, 소방서 마커 |
| F2 | 화재 확산 시뮬레이션 | 4~6주 | 대기 | 시나리오 POST API, 타임라인 애니메이션 |
| F3 | 대피로 계산 | 3~4주 | 대기 | pgRouting 경로 API, 3D 화살표 |
| F4 | 실시간 기상 연동 | 2~3주 | 대기 | 기상청 API, WebSocket 업데이트 |
...
```
총점 = 구조점수(목조+10, 조적+7, RC+3, SRC+0)
     + 노후도점수(30년 초과+10, 20년+7, 10년+3, 신축+0)
     + 용도점수(창고/공장+10, 다중이용+8, 주거+5, 업무+3)
     + 높이점수(고층(6F+)+8, 중층(3-5F)+5, 저층(1-2F)+3)
```
...
| 번호 | 태스크 | 예상 소요 | 담당 영역 | 비고 |
|------|--------|----------|----------|------|
| F2-01 | `building_adjacency` 검증 및 가중치 추가 (거리·소재·노출면) | 2일 | DB | F1-01 기반 확장 |
| F2-02 | networkx 그래프 로드 (adjacency → 그래프 객체) | 2일 | 시뮬레이션 |  |
| F2-03 | BFS 확산 알고리즘 구현 (바람 방향·속도 보정) | 3일 | 시뮬레이션 | 단계별 확산 확률 계산 |
| F2-04 | `fire_scenario_results` 테이블 설계 및 생성 | 1일 | DB |  |
| F2-05 | Celery async 태스크 래퍼 + Redis 캐싱 | 2일 | 인프라 | 동일 파라미터 시나리오 캐시 |
| F2-06 | POST /api/v1/fire/scenario + GET /api/v1/fire/scenario/{id} | 2일 | API |  |
| F2-07 | 예상 피해 통계 계산 (건물 수, 연면적, 추정 인명 피해) | 1일 | API |  |
| F2-08 | 프론트엔드: 바람 방향·속도 입력 UI | 2일 | Frontend |  |
| F2-09 | CesiumJS TimeIntervalCollection 타임라인 애니메이션 | 3일 | Frontend | 색상 단계 변화 포함 |
| F2-10 | 피해 통계 패널 (사이드바) | 1일 | Frontend |  |
...
| 소방서 API 응답 | 서울 전역 소방서·안전센터 119개 이상 조회 가능 |
