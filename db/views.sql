...
SELECT
    ...
    COALESCE(
        NULLIF(f.height, 0),
        NULLIF(l_best.bld_ht, 0),
        GREATEST(COALESCE(l_best.grnd_flr_cnt, f.grnd_flr, 3), 1) * 3.3,
        10.0
    ) AS height,
    ...
    COALESCE(l_energy.bld_area, l_best.bld_area, 0) AS building_area,
    ...
    CASE
        WHEN COALESCE(
            NULLIF(LEFT(NULLIF(COALESCE(l_energy.use_apr_day, l_best.use_apr_day, ''),''), 4),''),
    ...
    CASE
        ...
        WHEN l_best.strct_nm LIKE '%조적%' OR l_best.strct_nm LIKE '%벽돌%' OR l_best.strct_nm LIKE '%목%' THEN 'masonry'
        ELSE 'RC'
